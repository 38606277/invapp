...
export default class ChatNew extends React.Component {
  ...
  componentDidMount() {
    ...
    if (undefined != userInfo && null != userInfo && '' != userInfo) {
      this.setState({
        ...
        let mInfo = {
          'from_userId': this.state.userId, 'to_userId': this.state.to_userId,
  ...
  loadQuestion() {
    ...
    HttpService.post('/reportServer/questions/getQuestionsList', JSON.stringify(listParam))
      .then(resQes => {
        this.setState({
          questionList: resQes.list, data: [...this.state.data, { data: resQes.list, message_type: "question" }]
        }, function () {
          anchorElement.scrollIntoView();
        });
      })
  ...
  async sendMessage() {
    if (null != this.state.meg && "" != this.state.meg) {
      ...
      //计算答案
      await HttpService.post('/reportServer/nlp/getResult/' + this.state.userId + '/' + message, null)
        .then(res => {
          if (res.resultCode == "1000") {
            this.setState({
              data: [...this.state.data, {
